    ...
    return (
        <div className="dropdown">
            <button className="btn btn-black dropdown-toggle me-3 text-white" style={{position: "relative"}} data-bs-toggle="dropdown" aria-expanded="false">
                Favorites 
                <span className="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-warning text-black" style={{transform: "translate(-50%, -50%)"}}>
            ...
            <ul className="dropdown-menu dropdown-menu-black dropdown-menu-end me-3">
                ...
                : (store.favorites.map((item, id) => (
                        <li key={id} className="d-flex align-items-center me-2">
                            <span className="dropdown-item btn m-2" onClick={() => actions.removeFavorite(item)} href="#"> {item} </span>
